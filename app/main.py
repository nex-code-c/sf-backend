...
from app.routers import addresses, contacts
...
TAGS_METADATA = [
    ...
    },
    {
        "name": "addresses",
        "description": (
            "A contact's postal addresses. Each one is a separate record with its own "
            "`Home`/`Work`/`Other` type, so a contact can hold as many as it needs."
        ),
    },
    {
        "name": "meta",
